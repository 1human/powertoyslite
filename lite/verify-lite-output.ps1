...
if ($perUserPackages.Count -eq 0) {
    Write-Host "  [FAIL] 在 $installerRoot 下没找到每用户安装包" -ForegroundColor Red
    $script:Failures.Add('未生成每用户安装包') | Out-Null
} else {
    foreach ($file in $perUserPackages) {
